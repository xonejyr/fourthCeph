...
try:
    ...
    trainer_module = importlib.import_module(f"{MODULE_NAME}.trainer_only_new_pts_on_img")
    builder_module = importlib.import_module(f"{MODULE_NAME}.builder")
    visualize_distribution_heatmap = trainer_module.visualize_keypoints
...
def setup_output_directory(opt, cfg):
    ...
    heatmap_dir = os.path.join(output_vis_dir, 'pts_on_img')
    ...
    for filename in os.listdir(json_dir):
        if filename.endswith('_data.json'):
            try:
                ...
                logger.info(f"已生成热图: {filename}")
            except Exception as e:
                logger.error(f"为{filename}生成热图时出错: {e}")
