...
def im_to_torch(img):
    ...
    if img.max() > 1: # if not normlized, normalize it
        img /= 255
    return img
...
def heatmap_to_coord_simple(hms, bbox, **kwargs):
    ...
    coords, maxvals = get_max_pred(hms) # get the max of the heatmap as the coordinates

    # the original dimensions of the heatmap, [num_joints, hm_h, hm_w]
    hm_h = hms.shape[1]
    hm_w = hms.shape[2]
...
def heatmap_to_coord_medical(hms, **kwargs):
    """ 先在hm_shape上进行0.25的的细化， 再映射到IMAGE_SIZE(4*hm_size)尺寸，相当于在IMAGE_SIZE上调整了1个像素 """
    if not isinstance(hms, np.ndarray):
        hms = hms.cpu().data.numpy()
    coords, maxvals = get_max_pred(hms) # [num_joints, 2]
    ...
    # transform scale, different from the simple
    w = hm_w * 4
    h = hm_h * 4
    ...
    return preds[None, :, :] # None 表示在输出的第一个维度上增加一个大小为 1 的轴 [1, num_joints, 2]

def heatmap_to_coord(pred_jts, hm_shape):
    """ 先放大为4*hm_shape尺寸,也即IMAGE_SIZE尺寸，然后IMAGE_SIZE到IMAGE_SIZE的一个映射（没有意义） """
...
class get_coord(object):
    def __init__(self, cfg, norm_size):
        ...
        #self.input_size = cfg.DATASET.PRESET.RAW_IMAGE_SIZE
        self.norm_size = norm_size # output_size, usually heatmap_size, 而heatmap_size是IMAGE_SIZE的1/4 * 1/4

    def __call__(self, output, idx):
        if self.type == 'coord':
            pred_jts = output.pred_pts[idx] # only a sample, [num_joints, 2]
            return heatmap_to_coord(pred_jts, self.norm_size)
        elif self.type == 'heatmap':
            pred_hms = output.heatmap[idx] # only a sample, [num_joints, heatmap_size, heatmap_size]
