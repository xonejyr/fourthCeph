...
from nfdp.utils import DataLogger, scale_to_targetsize, visualize_heatmaps
from nfdp.metrics.mean_radial_error import mean_radial_error
from nfdp.metrics.successful_detection_rate import successful_detection_rate
...
def train(opt, cfg, train_loader, m, criterion, optimizer):
    ...
    for i, (inps, labels, _) in enumerate(train_loader):
        ...
        # print(len(labels))
        output = m(inps, labels) # forward pass
...
def validate(opt, cfg, val_loader, m, heatmap_to_coord, show_hm=False):
    ...
    for inps, labels, img_ids in val_loader:
        ...
        from nfdp.utils import get_center_scale, transform_preds
        ...
        if cfg.MODEL.TYPE == 'DualUNet_onlyBone':
            output['pred_pts_bone'][:, :, 0] = (output['pred_pts_bone'][:, :, 0] + 0.5) * HEATMAP_SIZE_W
            output['pred_pts_bone'][:, :, 1] = (output['pred_pts_bone'][:, :, 1] + 0.5) * HEATMAP_SIZE_H
            ...
            output['pred_pts_soft'][:, :, 0] = (output['pred_pts_soft'][:, :, 0] + 0.5) * HEATMAP_SIZE_W
            output['pred_pts_soft'][:, :, 1] = (output['pred_pts_soft'][:, :, 1] + 0.5) * HEATMAP_SIZE_H
            ...
            output['pred_pts'][:, :, 0] = (output['pred_pts'][:, :, 0] + 0.5) * HEATMAP_SIZE_W
            output['pred_pts'][:, :, 1] = (output['pred_pts'][:, :, 1] + 0.5) * HEATMAP_SIZE_H
        ...
        if cfg.MODEL.TYPE == 'DualUNet_onlyBone':
            ...
            gt_pts_rawImage_SIZE = np.zeros((BATCHSIZE, cfg.DATASET.PRESET.NUM_JOINTS, 2))  # [batch_size, num_keypoints, 2]
