...
from Unet.utils import Softmax_Integral, get_center_scale, transform_preds
...
class RefinementStep(nn.Module):
    def __init__(self, graph_channels, num_joints, hm_height_dim, hm_width_dim, 
        ...
        self.fusion_weights = nn.Sequential(
            nn.Linear(graph_channels * 2, 128),
            nn.ReLU(),
            nn.Linear(128, 2),
            nn.Softmax(dim=-1)
        )

        self.adj_proj = nn.Linear(graph_channels + 2, graph_channels)
        self.adj_attention = nn.Linear(graph_channels * 2, 1)
    ...
    def _map_coords_to_raw_image(self, coords, raw_image_size):
        heatmap_size = (self.hm_height_dim, self.hm_width_dim)
        hm_h, hm_w = heatmap_size
        raw_h, raw_w = raw_image_size
        ...
        coords_raw = torch.zeros_like(coords, device=coords.device)
        coords_np = coords.cpu().numpy()
        for b in range(B):
            for n in range(N):
                coords_raw[b, n] = torch.from_numpy(
                    transform_preds(coords_np[b, n], center, scale, [hm_w, hm_h])
                ).to(coords.device)
        return coords_raw

    def _generate_cropped(self, x, coords, step_idx, raw_image_size):
        B, N = coords.shape[0], coords.shape[1]
        coords_raw = self._map_coords_to_raw_image(coords, raw_image_size)

        crop_sizes = [64, 32, 16]
        size = crop_sizes[step_idx]
        half_size = size // 2
        cropped = torch.zeros(B, N, size, size, 3, device=x.device)

        x = x.permute(0, 2, 3, 1)  # [B, H, W, 3]
        raw_h, raw_w = raw_image_size
        for b in range(B):
            for n in range(N):
                x_c, y_c = coords_raw[b, n, 0].item(), coords_raw[b, n, 1].item()
                x_c, y_c = int(x_c), int(y_c)
                y_start = max(0, y_c - half_size)
                y_end = min(raw_h, y_c + half_size)
                x_start = max(0, x_c - half_size)
                x_end = min(raw_w, x_c + half_size)
                crop = x[b, y_start:y_end, x_start:x_end, :]
                pad_top = half_size - (y_c - y_start) if y_c - half_size < 0 else 0
                pad_bottom = half_size - (y_end - y_c) if y_c + half_size > raw_h else 0
                pad_left = half_size - (x_c - x_start) if x_c - half_size < 0 else 0
                pad_right = half_size - (x_end - x_c) if x_c + half_size > raw_w else 0
                crop_padded = F.pad(crop, (pad_left, pad_right, pad_top, pad_bottom), mode='constant', value=0)
                crop_padded = F.interpolate(crop_padded.permute(2, 0, 1).unsqueeze(0), size=(size, size), mode='bilinear', align_corners=True)
                cropped[b, n] = crop_padded.squeeze(0).permute(1, 2, 0)

        return cropped

    def _process_cropped_features(self, cropped):
        B, N = cropped.shape[0], cropped.shape[1]
        crop = cropped.view(B * N, cropped.shape[2], cropped.shape[3], 3).permute(0, 3, 1, 2)
    ...
    def _build_adjacency(self, coords, multi_scale_feats):
        B, N = coords.shape[0], coords.shape[1]
        multi_feats = self._sample_multi_scale_features(multi_scale_feats, coords)
        combined = torch.cat([multi_feats, coords], dim=-1)
        feat = self.adj_proj(combined)
        feat_i = feat.unsqueeze(2).expand(-1, -1, N, -1)
        feat_j = feat.unsqueeze(1).expand(-1, N, -1, -1)
        attn_input = torch.cat([feat_i, feat_j], dim=-1)
        adj = self.adj_attention(attn_input).squeeze(-1)
        adj = torch.sigmoid(adj)
        return adj

    def forward(self, coords, multi_scale_feats, x, step_idx, raw_image_size):
        if step_idx >= 3:
        ...
        # Generate cropped features dynamically for this step
        cropped = self._generate_cropped(x, coords, step_idx, raw_image_size)
        ...
        bone_cropped = self._process_cropped_features(cropped)[:, self.bone_indices]
        soft_cropped = self._process_cropped_features(cropped)[:, self.soft_indices]
        ...
        bone_weights = self.fusion_weights(bone_fusion_input)
        soft_weights = self.fusion_weights(soft_fusion_input)
        ...
        bone_mean = bone_gat_feats.mean(dim=-1).unsqueeze(2)
        soft_mean = soft_gat_feats.mean(dim=-1).unsqueeze(1)
        cross_diff = bone_mean - soft_mean
...
class HierarchicalGraphResFPNEnhancedMultiStepSample(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, **cfg):
        super(HierarchicalGraphResFPNEnhancedMultiStepSample, self).__init__()
        ...
        self.raw_image_size = self._preset_cfg['RAW_IMAGE_SIZE']
    ...
    def forward(self, x, labels=None):
        ...
        for i, step in enumerate(self.refine_steps):
            current_hm = step(coords, multi_scale_feats, x, step_idx=i, raw_image_size=self.raw_image_size)
