...
class DynamicUpsample(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, scale_factor=2):
        ...
        self.weight_gen = nn.Sequential(
            nn.Conv2d(in_channels, 128, 1), # 只改变通道
            nn.ReLU(),
            nn.Conv2d(128, out_channels // (scale_factor * scale_factor), 1)  # Match x's channels after pixel_shuffle， 只是改变通道
        )
...
class ResUNetFPN(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, **cfg):
        ...
        # 这里的high_channels和upsample的输出通道数对齐，作为x_high channels, 
        # 第一个是齐平，
        # 这里的前两个通道，逐次递减2即可
        self.msfe1 = MSFE(1024, 1024, high_channels=512)   # x_high=256 from feats_fpn[0]
        self.msfe2 = MSFE(1024, 512, high_channels=256)   # x_high=32 from upsample2 self-fusion
        self.msfe3 = MSFE(512,  256, high_channels=128)    # x_high=16 from upsample3 self-fusion

        # 1.输入通道，2.输出通道/4，3.尺度x2, 这里的（1，1）与最深层的特征的通道数对齐， 然后逐2递减
        # 第一个是气平，且是高维
        # 
        self.upsample1 = DynamicUpsample(2048, 2048)     # 2048 -> 256
        self.upsample2 = DynamicUpsample(1024, 1024)     # 1024 -> 128
        self.upsample3 = DynamicUpsample(512, 512)      # 512 -> 64
        ...
        self.upconv = nn.ConvTranspose2d(
            ...
            out_channels=2048,   # 输出通道数（保持不变）
            kernel_size=4,       # 核大小，通常为 2 或 4
            stride=2,            # 步幅为 2，使空间尺寸翻倍
            padding=1            # 调整 padding 以精确控制输出尺寸
            )
    ...
    def forward(self, x, labels=None):
        ...
        # UNet Decoder 先FPN
        in4 = self.upsample1(feats[3])  # [8, 2048, 16, 16] => [8, 512, 32, 32]
        d3 = self.msfe1(in4, feats[2]) # ([8, 1024, 32, 32], [8, 512, 32, 32]) => [8, 1024, 32, 32]

        in3 = self.upsample2(feats[2])  # [8, 1024, 32, 32] => [8, 256, 64, 64] 
        d2 = self.msfe2(in3, d3)  # ([8, 256, 64, 64], [8, 1024, 32, 32]) => [8, 1024, 64, 64]
        in2 = self.upsample3(feats[1])  # [8, 512, 64, 64] => [8, 64, 128, 128] 
        # 移除对feats[0]的依赖，直接上采样
        d1 = self.msfe3(in2, d2)  # ([8, 64, 128, 128], [8, 256, 64, 64], ) => [8, 128, 128, 128]
        #in1 = self.upsample3(feats[0])  
        #d0 = self.msfe3(d1, in1)  
        #d_out = self.up(d1) # [8, 256, 256, 256] => [8, 128, 512, 512]

        #d4 = self.upconv(d4)

        feat2fpnhead = (d1, d2, d3, feats[3])
        # [8, 2048, 16, 16]
        # [8, 1024, 32, 32]
        # 
        feat2fpnhead = self.neck(feat2fpnhead)
        output_hm = self.head(feat2fpnhead)
