...
class HeatmapBasisNFRwithOrth(nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, **cfg):
        super(HeatmapBasisNFRwithOrth, self).__init__()
    ...
    def forward(self, x, labels=None):
        ...
        if self.training and labels is not None:
            ...
            gt_uv_weight = labels['target_uv_weight'].reshape(pred_pts.shape)
            ...
            losses.basis_loss = -log_p.unsqueeze(2) * gt_uv_weight[:, :, :1]

            # 多样性正则化
            losses.div_loss = torch.zeros(1, device=x.device)
            for k in range(self.basis_dist.num_bases):
                for j in range(k + 1, self.basis_dist.num_bases):
                    dist_k = basis_dists[k]
                    dist_j = basis_dists[j]
                    losses.div_loss += torch.distributions.kl_divergence(dist_k, dist_j).mean()

            # 参数正则化
            losses.reg_loss = sum(p.pow(2).sum() for p in self.basis_dist.parameters())
        ...
        output = EasyDict(
            ...
            losses=losses if self.training and labels is not None else None
        )
        return output
