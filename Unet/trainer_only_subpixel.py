""" can do with subpixel repair for ResNet """
import json
import os
import pickle as pk
import numpy as np
...
def validate(opt, cfg, val_loader, m, heatmap_to_coord, show_hm=False):
    ...
    for inps, labels, img_ids in val_loader:
        ...
        if show_hm and batchCnt < batchNumVisual:
            ...
            #if cfg.MODEL.TYPE == 'DualUNet_onlyBone':
            #    visualize_heatmaps(opt, inps, labels['target_hm_bone'], output['heatmap_bone'], img_ids, target_uv=labels['target_uv_bone'].reshape(BATCHSIZE,-1, 2), pred_uv=output['pred_pts_bone'], type=cfg.DATASET.PRESET.METHOD_TYPE, show_norm=cfg.VALIDATE.SHOWNORM)
            #elif cfg.MODEL.TYPE == 'DualUNet_onlySoft':
            #    visualize_heatmaps(opt, inps, labels['target_hm_soft'], output['heatmap_soft'], img_ids, target_uv=labels['target_uv_soft'].reshape(BATCHSIZE,-1, 2), pred_uv=output['pred_pts_soft'], type=cfg.DATASET.PRESET.METHOD_TYPE, show_norm=cfg.VALIDATE.SHOWNORM)
            #else:
        ...
        pose_coords_batch = torch.zeros_like(output['pred_pts'])
        for i in range(BATCHSIZE):
            pose_coords = heatmap_to_coord(
                output, idx=i) # 坐标在IMAGE_SIZE尺寸上的值(all); output为一个样本而非一个batch，输出为[1,num_joints, 2]; 亚像素修正主要是为了修正pred_pts,
            pose_coords_batch[i] = torch.from_numpy(pose_coords) #最终得到的是[batch_size, num_joints, 2], 亚像素调整后，且为 IMAGE_SIZE 尺寸

        from Unet.utils import get_center_scale, transform_preds

        # 对于ResNet, 它的 HEATMAP_SIZE 是 IMAGE_SIZE 的 1/4, 
        # 亚像素变换将得到的结果坐标 pose_coords （不论是coord还是heatmap）都亚像素调整到HEATMAP_SIZE的尺寸上
        # 所以这里的映射，不再是 ResFPN 的HEATMAP_SIZE，而是其4*4，是 IMAGE_SIZE
        HEATMAP_SIZE_H, HEATMAP_SIZE_W = cfg.DATASET.PRESET.IMAGE_SIZE # check here, not HEATMAP_SIZE
        ...
        #if cfg.MODEL.TYPE == 'DualUNet_onlyBone':
        #    output['pred_pts_bone'][:, :, 0] = (output['pred_pts_bone'][:, :, 0] + 0.5) * HEATMAP_SIZE_H
        #    output['pred_pts_bone'][:, :, 1] = (output['pred_pts_bone'][:, :, 1] + 0.5) * HEATMAP_SIZE_W
        #    output['pred_pts_bone'].cpu().numpy()
        #    pred_pts_rawImage_SIZE = np.zeros((BATCHSIZE, cfg.DATASET.PRESET.NUM_JOINTS_BONE, 2))  # [batch_size, num_keypoints, 2] of rawImage_SIZE
        #elif cfg.MODEL.TYPE == 'DualUNet_onlySoft':
        #    output['pred_pts_soft'][:, :, 0] = (output['pred_pts_soft'][:, :, 0] + 0.5) * HEATMAP_SIZE_H
        #    output['pred_pts_soft'][:, :, 1] = (output['pred_pts_soft'][:, :, 1] + 0.5) * HEATMAP_SIZE_W
        #    output['pred_pts_soft'].cpu().numpy()
        #    pred_pts_rawImage_SIZE = np.zeros((BATCHSIZE, cfg.DATASET.PRESET.NUM_JOINTS_SOFT, 2))  # [batch_size, num_keypoints, 2] of rawImage_SIZE
        #else:
        #output['pred_pts'][:, :, 0] = (output['pred_pts'][:, :, 0] + 0.5) * HEATMAP_SIZE_H
        #output['pred_pts'][:, :, 1] = (output['pred_pts'][:, :, 1] + 0.5) * HEATMAP_SIZE_W
        pose_coords_batch.cpu().numpy() # 原先不清楚device，是np.array(), 亚像素调整的预测坐标，IMAGE_SIZE 
        pred_pts_rawImage_SIZE = np.zeros((BATCHSIZE, cfg.DATASET.PRESET.NUM_JOINTS, 2))  # [batch_size, num_keypoints, 2] of rawImage_SIZE
        
        
        for i in range(BATCHSIZE):
            #if cfg.MODEL.TYPE == 'DualUNet_onlyBone':
            #    for j in range(cfg.DATASET.PRESET.NUM_JOINTS_BONE):
            #        pred_pts_rawImage_SIZE[i, j, :] = transform_preds(output['pred_pts_bone'][i, j, :].cpu().numpy(), center, scale, [HEATMAP_SIZE_W, HEATMAP_SIZE_H]) # 一个x,y坐标对
            #elif cfg.MODEL.TYPE == 'DualUNet_onlySoft':
            #    for j in range(cfg.DATASET.PRESET.NUM_JOINTS_SOFT):
            #        pred_pts_rawImage_SIZE[i, j, :] = transform_preds(output['pred_pts_soft'][i, j, :].cpu().numpy(), center, scale, [HEATMAP_SIZE_W, HEATMAP_SIZE_H]) # 一个x,y坐标对
            #else:
            for j in range(cfg.DATASET.PRESET.NUM_JOINTS):
                pred_pts_rawImage_SIZE[i, j, :] = transform_preds(pose_coords_batch[i, j, :].cpu().numpy(), center, scale, [HEATMAP_SIZE_W, HEATMAP_SIZE_H]) # 一个x,y坐标对
        ...
        #if cfg.MODEL.TYPE == 'DualUNet_onlyBone':
        #    labels['target_uv_bone'] = scale_to_targetsize(labels['target_uv_bone'], cfg.DATASET.PRESET.IMAGE_SIZE) # [batch_size, num_keypoints * 2]
        #    labels['target_uv_bone'] = labels['target_uv_bone'].reshape(BATCHSIZE, -1, 2) # [batch_size, num_keypoints, 2]
        #    gt_pts_rawImage_SIZE = np.zeros((BATCHSIZE, cfg.DATASET.PRESET.NUM_JOINTS_BONE, 2))  # [batch_size, num_keypoints, 2] of rawImage_SIZE
        #elif cfg.MODEL.TYPE == 'DualUNet_onlySoft':
        #    labels['target_uv_soft'] = scale_to_targetsize(labels['target_uv_soft'], cfg.DATASET.PRESET.IMAGE_SIZE) # [batch_size, num_keypoints * 2]
        #    labels['target_uv_soft'] = labels['target_uv_soft'].reshape(BATCHSIZE, -1, 2) # [batch_size, num_keypoints, 2]
        #    gt_pts_rawImage_SIZE = np.zeros((BATCHSIZE, cfg.DATASET.PRESET.NUM_JOINTS_SOFT, 2))  # [batch_size, num_keypoints, 2]
        #else:
        ...
        gt_pts_rawImage_SIZE = np.zeros((BATCHSIZE, cfg.DATASET.PRESET.NUM_JOINTS, 2))  # [batch_size, num_keypoints, 2]，先IMAGE_SIZE有点不是很必要，但是不要改了
            

        for i in range(BATCHSIZE):
            #if cfg.MODEL.TYPE == 'DualUNet_onlyBone':
            #    for j in range(cfg.DATASET.PRESET.NUM_JOINTS_BONE):
            #        gt_pts_rawImage_SIZE[i, j, :] = transform_preds(labels['target_uv_bone'][i, j, :].cpu().numpy(), center, scale, [IMAGE_SIZE_W, IMAGE_SIZE_H]) # 一个x,y坐标对
            #elif cfg.MODEL.TYPE == 'DualUNet_onlySoft':
            #    for j in range(cfg.DATASET.PRESET.NUM_JOINTS_SOFT):
            #        gt_pts_rawImage_SIZE[i, j, :] = transform_preds(labels['target_uv_soft'][i, j, :].cpu().numpy(), center, scale, [IMAGE_SIZE_W, IMAGE_SIZE_H]) # 一个x,y坐标对
            #else:
            for j in range(cfg.DATASET.PRESET.NUM_JOINTS):
