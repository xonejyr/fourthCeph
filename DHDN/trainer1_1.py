...
from DHDN.utils import DataLogger, scale_to_targetsize, visualize_heatmaps
from DHDN.metrics.mean_radial_error import mean_radial_error
from DHDN.metrics.successful_detection_rate import successful_detection_rate
...
def train(opt, cfg, train_loader, m, criterion, optimizer):
    ...
    grad_clip = cfg.TRAIN.GRAD_CLIP.get('ENABLED', False) # default as False
    ...
    for i, (inps, labels, _) in enumerate(train_loader):
        ...
        # print(len(labels))
        output = m(inps, labels['target_uv']) # forward pass
...
def validate(opt, cfg, val_loader, m, heatmap_to_coord, show_hm=False):
    ...
    for inps, labels, img_ids in val_loader:
        ...
        output = m(inps, labels['target_uv']) # forward pass, output has two items, one for 'heatmap' and one for 'pred_pts'
