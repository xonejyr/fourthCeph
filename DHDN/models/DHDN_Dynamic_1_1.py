...
#################################################################################80
## Dynamic_1_1
# 1.points 增加了高斯噪声做数据增强
# 2.更换多种 lr_scheduler, 默认为 CosineAnnealingLR
# 3.几何约束 chamfer_distance
# 4.hypergraph 的层数可配置，并增多
# 5.一律引入梯度裁剪: l2, max_norm=1



@MODEL.register_module
class DHDN_Dynamic_1_1(nn.Module):
    def __init__(self, **cfg):
        # num_joints=19, num_features=2, num_patterns=5, hidden_dim=64, image_size_h=256, image_size_w=256):
        super(DHDN_Dynamic_1_1, self).__init__()
        ...
        self.points_sigma = cfg['POINTS_SIGMA'] # 点云的方差
        self.num_layers = cfg['NUM_LAYERS'] # 超图卷积的层数
        ...
        self.hyper_gnn = nn.ModuleList([
            pyg_nn.HypergraphConv(self.hidden_dim, self.hidden_dim) for _ in range(self.num_layers)  # 3 层超图卷积
        ])
        self.hyper_gnn_norms = nn.ModuleList([
            nn.LayerNorm(self.hidden_dim) for _ in range(self.num_layers)  # 每层加归一化
        ])
    ...
    def forward(self, img, target_uv):
        ...
        noise = torch.randn_like(points) * self.points_sigma
        points = points + noise
